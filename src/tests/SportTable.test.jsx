...
describe('SportTable', () => {
    ...
    it('displays data from database correctly', async () => {
        ...
        await waitFor(() => {
            // check sport data
            expect(screen.getByText((content, element) => content.startsWith('sport_type1'))).toBeInTheDocument();
            expect(screen.getByText((content, element) => content.startsWith('sport_type2'))).toBeInTheDocument();
            // check medal
    ...
    it('display unavailable message when no data in database', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Schedule is unavailable at the moment.')).toBeInTheDocument();
        });
    });
